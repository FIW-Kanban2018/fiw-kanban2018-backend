...
public interface DringendTaskRepository extends GenericTaskRepository<DringendTaskEntity> {
    ...
    @Query(value = "SELECT * FROM dringend d WHERE t.category = 'geschaeft'",
            nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM dringend d WHERE t.category = 'langfristig'",
            nativeQuery = true)
    List<Object> findAllByCardCategoryLang();

    @Query(value = "SELECT * FROM dringend d WHERE t.category = 'done'",
            nativeQuery = true)
